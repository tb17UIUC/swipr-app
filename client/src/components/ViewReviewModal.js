...
const ViewReviewModal = ({ open, onCancel, clothing }) => {
    ...
    return (
        <Dialog open={open} onClose={onCancel}>
            <DialogTitle>
                <strong>Reviews</strong> for {clothing?.Name} from{' '}
                <i>{clothing?.Brand}</i>
            </DialogTitle>
            <DialogContent>
                ...
                <div>
                    {reviews.length > 0 ? (
                        reviews.map((item, index) => (
                            <div
                                className="bg-tertiary text-white rounded-xl mb-4 p-4 relative"
                                key={index}
                            >
                                <p className="text-lg font-bold absolute top-2 left-4">
                                    Fit: {item.Fit}
                                </p>
                                <div className="absolute top-2 right-4">
                                    <div style={{ fontSize: '24px' }}>
                                        {[...Array(item.Star_Rating)].map(
                                            (_, i) => (
                                                <span
                                                    key={i}
