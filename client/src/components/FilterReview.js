...
const FilterReviewModal = ({filterReview}) => {
  // State variables to hold the values
  const [maxPrice, setMaxPrice] = useState(999); // Default value for maxPrice
  const [minPrice, setMinPrice] = useState(9.90); // Default value for minPrice
...
return (
  <div className="bg-primary text-white rounded-xl mb-4 p-6" style={{ height: '565px', width: '570px', fontSize: '1.2rem'}}>
    <h2 className="text-xlg font-bold text-center" style={{fontSize: '1.5rem'}}>Review Filters</h2>
    <div className="mb-1">
      ...
      />
      <div className="flex justify-between text-sm" style={{fontSize: '1.0rem'}}>
        <span>$9.90</span>
        <span>$999.00</span>
      </div>
    </div>
    ...
    <hr style={{ backgroundColor: 'white', height: '2px', border: 'none', margin: '10px 0' }} />

    <div>
  <label className="block mb-2">Brand:  </label>
  <select className="mb-2" value={brand} onChange={handleBrandChange} style={{ color: 'black' }}>
    ...
    <label className="block mb-2">
        Made in USA:
        <input type="checkbox" checked={madeInUSA} onChange={handleMadeInUSAChange} style={{ marginLeft: '0.5rem', transform: 'scale(1.5)' }} />
    </label>
    </div>
    ...
    <label className="block mb-2">
        Minority Owned:  
        <input type="checkbox" checked={minorityOwned} onChange={handleMinorityOwnedChange} style={{ marginLeft: '0.5rem', transform: 'scale(1.5)' }} />
    </label>
    </div>
    ...
    <div className="mb-6">
      ...
      <div className="flex justify-center items-center">
                <button className="bg-white text-primary rounded hover:bg-tertiary h-10 w-40 text-lg px-4"
                    onClick={() => filterReview(FilterArray)}
                >
